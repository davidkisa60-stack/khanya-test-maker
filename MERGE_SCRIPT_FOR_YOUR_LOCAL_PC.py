...
Place this file in the same folder as your subjects/ directory
(or run it from the root of your math_paper_site folder).

Usage examples:
    python MERGE_SCRIPT_FOR_YOUR_LOCAL_PC.py --subject biology --paste-file cleaned.json

    python MERGE_SCRIPT_FOR_YOUR_LOCAL_PC.py --subject mathematics
    (will prompt you to paste the JSON array)

This is the ONLY script you need to update on your local computer
for the multi-subject system.
"""
...
def merge_questions(current_questions, new_questions, subject_id):
    ...
    for q in new_questions:
        year = q.get("year")
        paper = q.get("paper", "")
        original_num = q.get("original_num", q.get("id", "").split("-")[-1] if "-" in q.get("id", "") else q.get("id"))
...
def main():
    ...
    parser.add_argument("--subject", required=True, help="Subject id (e.g. biology, mathematics, physical_science)")
    ...
    if args.paste_file:
        ...
        print("\nPaste the cleaned JSON array below (end with a blank line):")
        lines = []
        while True:
            try:
                line = input()
    ...
    print(f"\nNext step: Hard refresh your browser (Ctrl+Shift+R) and switch to the '{args.subject}' subject in the UI.")
