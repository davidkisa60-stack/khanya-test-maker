...
// Reveal/hide must run first. If there is no logged-in user, go to /login.
(function() {
    var user = null;
    try {
...
const BACKEND_URL = "";   // same-origin (Render hosts frontend + API together)
...
window.getAllowedSubjects = getAllowedSubjects;
window.canAccessSubject = canAccessSubject;
...
async function showSubject(subject) {
    if (!canAccessSubject(subject)) {
        alert('You do not have access to this subject. Please contact the administrator.');
        showSubjects();
        return;
    }
...
function goToSubjects(e) {
    if (e) {
        e.preventDefault();
        e.stopPropagation();
    }
    console.log('%c[Khanya] New Test clicked', 'color:#0ea5e9');
    const fn = window.showSubjects || showSubjects;
    if (typeof fn === 'function') fn();
    else alert('App is still loading. Please wait a second and try again.');
}
window.goToSubjects = goToSubjects;

function attachNewTestButtons() {
    ['btn-new-test-nav', 'btn-new-test-big'].forEach(id => {
        const btn = document.getElementById(id);
        if (!btn) return;
        btn.setAttribute('type', 'button');
        btn.onclick = goToSubjects;
    });
}
